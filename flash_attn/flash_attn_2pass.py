...
@triton.jit
def _attn_fwd(Q, K, V, sm_scale, M, L, Out, C, # C = (Z, H, N_CTX)
...
def test_op(Z, H, N_CTX, HEAD_DIM, causal, dtype=torch.float16):
    ...
    # row_max = torch.max(p, dim=-1, keepdim=True).values
    # exp_shifted = torch.exp(p - row_max)
    # row_sum = torch.sum(exp_shifted, dim=-1, keepdim=True)
    # lse = torch.log(row_sum) + row_max.squeeze(-1)
    # m_ref = exp_shifted / row_sum
    
    p = torch.softmax(p.float(), dim=-1).half()
    ref_c = torch.sum(p, dim=2)
    ...
    print("-------TRITON------")
    print(tri_c)
    assert torch.allclose(ref_c, tri_c, atol=1e-2, rtol=0)
    
# def run_test():
#     Z, H, N_CTX, HEAD_DIM = 1, 1, 128, 64
#     causal = False
#     test_op(Z, H, N_CTX, HEAD_DIM, causal)
#     print("Test Passed!")
