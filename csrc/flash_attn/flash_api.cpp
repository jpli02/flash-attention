...
void set_params_fprop_accum(Flash_fwd_params &params,
                      // sizes
                      const size_t b,
                      const size_t seqlen_q,
                      const size_t seqlen_k,
...
void run_mha_fwd_accum(Flash_fwd_params &params, cudaStream_t stream, bool force_split_kernel=false) {
    FP16_SWITCH(!params.is_bf16, [&] {
        HEADDIM_SWITCH(params.d, [&] {
            if (params.num_splits <= 1 && !force_split_kernel) {  // If we don't set it num_splits == 0
                run_mha_fwd_accum_<elem_type, kHeadDim>(params, stream);
            } else {
                // run_mha_fwd_splitkv_dispatch<elem_type, kHeadDim>(params, stream);
            }
        });
...
std::vector<at::Tensor>
mha_fwd_accum(at::Tensor &q,         // batch_size x seqlen_q x num_heads x head_size
        const at::Tensor &k,         // batch_size x seqlen_k x num_heads_k x head_size
        const at::Tensor &v,         // batch_size x seqlen_k x num_heads_k x head_size
        c10::optional<at::Tensor> &out_,             // batch_size x seqlen_q x num_heads x head_size
        c10::optional<at::Tensor> &alibi_slopes_, // num_heads or batch_size x num_heads
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("fwd_accum", &mha_fwd_accum, "Forward pass with accumulated attention score");
